...
class PhasedMeanReversionAccumulator(ScriptStrategyBase):
    ...
    base_asset = trading_pair.split("-")[0]  # BA
    quote_asset = trading_pair.split("-")[1]  # QA
    ...
    def _execute_ba_accumulation(self):
        ...
        if self.ba_state == "idle" and self.ba_avg_bid:
            ...
            if ba_balance >= self.ba_amount_to_sell:
                ...
                if self.ba_sell_order_id:
                    ...
                    self.logger().info(f"BA: Started transformation phase - Selling {self.ba_amount_to_sell} at {self.ba_sell_price}")
        ...
        # Calculation Phase (Step 3) - triggered after sell order fills
            ...
            self.logger().info(f"BA: Calculation complete - Will buy back at {self.ba_buy_price}")
        ...
        # Going Back Phase (Step 4)
            ...
            if max_buy_amount >= self.ba_amount_to_sell:  # Buy back what we sold plus more
                ...
                if self.ba_buy_order_id:
                    self.logger().info(f"BA: Going back phase - Buying {buy_amount} at {self.ba_buy_price}")
    ...
    def _execute_qa_accumulation(self):
        ...
        if self.qa_state == "idle" and self.qa_avg_ask:
            ...
            if qa_balance >= self.qa_amount_to_spend:
                ...
                if self.qa_buy_order_id:
                    ...
                    self.logger().info(f"QA: Started transformation phase - Buying {ba_amount_to_buy} BA at {self.qa_buy_price}")
        ...
        # Calculation Phase (Step 3) - triggered after buy order fills
            ...
            self.logger().info(f"QA: Calculation complete - Will sell at {self.qa_sell_price}")
        ...
        # Going Back Phase (Step 4)
            ...
            if ba_balance > Decimal("0"):
                ...
                if self.qa_sell_order_id:
                    self.logger().info(f"QA: Going back phase - Selling {sell_amount} at {self.qa_sell_price}")
    
    def did_fill_order(self, event: OrderFilledEvent):
        """Handle order fill events"""
        # BA accumulation events
        if event.order_id == self.ba_sell_order_id:
            self.logger().info(f"BA: Transformation sell order filled at {event.price}")
            ...
            self.logger().info(f"BA: Going back buy order filled at {event.price}")
            self.ba_state = "idle"
        ...
        # QA accumulation events
            ...
            self.logger().info(f"QA: Transformation buy order filled at {event.price}")
            ...
            self.logger().info(f"QA: Going back sell order filled at {event.price}")
            self.qa_state = "idle"
    ...
    def format_status(self) -> str:
        ...
        if self.ba_avg_bid and self.qa_avg_ask:
            ...
            lines.append(f"Spread: {(self.qa_avg_ask - self.ba_avg_bid) / self.ba_avg_bid * 100:.2f}%")
        lines.append("")
        ...
        ba_balance = self.connectors[self.exchange].get_balance(self.base_asset)
        qa_balance = self.connectors[self.exchange].get_balance(self.quote_asset)
        lines.append(f"Balances - {self.base_asset}: {ba_balance:.8f} | {self.quote_asset}: {qa_balance:.2f}")
