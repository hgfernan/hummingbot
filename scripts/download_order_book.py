...
def log_path() -> str:
    return str(DEFAULT_LOG_FILE_PATH)
...
class DownloadOrderBookSnapshots(ScriptStrategyBase):
    ...
    def init_markets(cls, config: BaseModel) -> None:
        ...
        cls.logger().info("Initialized markets for %s and %s", cls.exchange, cls.trading_pair)
    ...
    def on_tick(self) -> None:
        ...
        if self.dump_count < self.n_dumps:
            order_book_data: Dict[str, object] = self.get_order_book_dict(self.exchange, self.trading_pair, self.depth)

            if not order_book_data["bids"] and not order_book_data["asks"]:
                self.logger().warning("No order book data available for %s on %s", self.trading_pair, self.exchange)
                return

            if self.dump_count == 0:
                self.logger().info(
                    "Starting to collect order book snapshots for %s on %s", self.trading_pair, self.exchange
                )
                # self.logger().info(pprint.pformat(dir(order_book_data)))

            # HINT append the order book data to the temporary storage
            self.ob_temp_storage = pd.concat([self.ob_temp_storage, pd.DataFrame([order_book_data])], ignore_index=True)

            self.logger().info(
                "Collected order book snapshot %d for %s on %s", self.dump_count + 1, self.trading_pair, self.exchange
            )
    ...
    def get_order_book_dict(self, exchange: str, trading_pair: str, depth: int = 50) -> Dict[str, object]:
        """
